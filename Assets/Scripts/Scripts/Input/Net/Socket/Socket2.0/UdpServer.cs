    ...
    public class UdpServer : IJNet
    {
        ...
        void OnRun()
        {
            ...
            finally
            {
                ...
                {
                    isRun = true;
                    eventQueue.Enqueue(() => { m_serverEvent.OnRun(); });
        ...
        void Error(string msg, string type) { eventQueue.Enqueue(() => { m_serverEvent.OnError(msg, type); }); }
        ...
        {
            this.m_serverEvent = (IUDPEvent)netParams.NetEvent;
        ...
        public void Update()
        {
            ...
            if (isRun)
            {
                while (msgPools.Count > 0)
                    m_serverEvent.OnMessage(msgPools.Dequeue());
